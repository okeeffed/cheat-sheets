...
`ohai` gives back all the node system data in JSON. Everytime `chef-client` is run, it automatically executes an `ohai` and allows this object (known as the node object) to be accessed.
